  ...
  values = [
    ...
    <<-YAML
      ingress:
        ...
        # (not the viewer-facing dev.lodge104.net) for custom origins, so the
        # ALB needs a matching rule for it too or it falls through to the
        # default 404 fixed-response.
        extraHosts:
          - name: origin.dev.lodge104.net
            path: /
    YAML
    ,
    <<-YAML
